...
class TextNoteCreator(object):
    ...
    def _get_position_data(self, is_left_side, bbox, offset_vector):
        ...
        if self.view_orientation == ViewOrientation.SECTION:
            if self.active_view.RightDirection.Y < 0:
                if is_left_side:
                    return {
                        'position': XYZ(bbox.Max.X, bbox.Max.Y, bbox.Min.Z) + offset_vector,
                        'alignment': HorizontalTextAlignment.Left,
                        'leader_end': XYZ(bbox.Max.X, bbox.Max.Y, bbox.Min.Z)
                    }
                return {
    ...
    def create_text_note(self, element, text=None):
